...
| 快捷键 | 功能 |
|--------|------|
| `Ctrl+R` | 对当前序列候选帧（`HALLUCINATION` / `DUPLICATE` / `SIMILAR`）执行批量 paraphrase 改写 |
...
| 入口 | 位置 | 说明 |
|------|------|------|
| 批量改写按钮 | 右侧「问题列表」底部 | 对当前序列候选帧（幻觉/重复/相似）批量执行改写 |
...
| 阶段 | 标记状态 |
|------|----------|
| 改写前 | `HALLUCINATION` 或自动违规候选（`DUPLICATE` / `SIMILAR`） |
| 改写成功 | 若原为 `HALLUCINATION` 则移除该标记；统一新增 `AI_GENERATED` |
| 改写失败/跳过 | 保持原状态（手动标记/自动违规均不改变）|
