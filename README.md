...
- **[全新功能：Paraphrase 批量改写](#14-paraphrase-批量改写功能)**：新增针对 `HALLUCINATION` 帧的一键批量改写能力（按钮入口 + `Ctrl+R` 快捷键）
- **[新功能请求链路](#144-请求链路端到端)**：`paraphrase` 统一走 Anthropic SDK（MiniMax Anthropic 兼容端点），自动解析返回中的 `text` content block
- **[新功能配置项](#143-配置项说明)**：新增并启用 paraphrase 相关配置（`paraphrase_model`、`minimax_api_key`、`openai_base_url`、`openai_api_key`、`openai_model`）
- **[Prompt 长度约束增强](#145-prompt-规则当前生效)**：改写结果新增硬性要求 **不超过 30 词**，并尽量控制在 **20 词以内**
- **[改写结果流转](#147-标记流转规则)**：改写成功后自动移除 `HALLUCINATION` 并写入 `AI_GENERATED` 标记，同时刷新违规检测与问题列表
- **[标记帧稳定性修复](#9-问题帧标记)**：修复 `FLAG_TYPES` 与弹窗文案不一致导致的“标记帧卡死/退出”问题（补齐 `AI_GENERATED` 并增加兜底文案）
- **[幻觉队列实时同步](#142-入口与触发方式)**：手动新增/取消 `HALLUCINATION` 后，批量改写队列立即更新，不再出现“当前序列没有幻觉帧”的误提示
- **[鼠标侧键翻帧](#11-快捷键参考)**：支持 `XButton1/XButton2` 直接切换上一帧/下一帧
- **[文本编辑体验优化](#6-编辑与保存逻辑)**：
...
14. [Paraphrase 批量改写功能](#14-paraphrase-批量改写功能)
15. [常见问题](#15-常见问题)
...
| anthropic | ≥ 0.40.0 | Paraphrase（Anthropic 兼容 API）|
...
| 快捷键 | 功能 |
|--------|------|
| `Ctrl+R` | 对当前序列所有 `HALLUCINATION` 帧执行批量 paraphrase 改写 |
...
> `V1.2` 新增能力。用于对人工标记为幻觉（`HALLUCINATION`）的帧执行批量改写，自动生成更稳健的替代表达。

### 14.1 功能定位

- 该功能是**新增模块**，在 `V1.1` 及更早版本中不存在。
- 目标：对同一序列中被标记为 `HALLUCINATION` 的文本，自动生成一条更可靠的 paraphrase。
- 改写完成后：
  - 原 `HALLUCINATION` 标记会被移除
...
| 入口 | 位置 | 说明 |
|------|------|------|
| 批量改写按钮 | 右侧「问题列表」底部 | 对当前序列幻觉帧批量执行改写 |
| `Ctrl+R` | 全局快捷键 | 弹出确认框后执行批量改写 |

执行前置条件：

1. 当前已加载序列；
2. 当前序列存在 `HALLUCINATION` 标记帧；
...
1. 用户点击批量改写（或 `Ctrl+R`）。
2. `ui/main_window.py` 收集当前序列的幻觉帧索引并启动 `_RewriteThread`（后台线程）。
3. 线程内按帧处理：
   - 从当前帧向前回溯，提取最多 3 条“非幻觉、非 AI 生成”的参考 caption；
...
|------|----------|
| 改写前 | `HALLUCINATION` |
| 改写成功 | 移除 `HALLUCINATION`，新增 `AI_GENERATED` |
| 改写失败/跳过 | 保持原状态（通常仍为 `HALLUCINATION`）|
